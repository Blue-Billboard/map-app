...
- [x] **Phase 3: Foundation** — Design tokens, Outfit font, zero shadows, custom step indicator, step transitions (completed 2026-03-03)
- [x] **Phase 4: Venue Selection** — Full-bleed hero cards, selected state, flat filter bar, count chip (Step 1) (completed 2026-03-03)
- [x] **Phase 5: Campaign Details + Mobile** — Flat discount cards, duration selector, venue chips, mobile full-screen (Step 2) (completed 2026-03-03)
- [x] **Phase 6: Quote & Pricing** — Featured tier block, flat pricing cards, large price display, CTA section, geometric decoration (Step 3) (completed 2026-03-05)
...
Plans:
- [ ] 04-01-PLAN.md — Venue card redesign: full-bleed 16:9 hero image, navy overlay band, flat selection state and square badge
- [ ] 04-02-PLAN.md — Filter bar and count chip: gray-100 flat container and solid navy count chip
...
Plans:
- [ ] 05-01-PLAN.md — Flat discount cards, duration selector, and venue chips (Step 2 CSS)
- [ ] 05-02-PLAN.md — Mobile full-screen wizard dialog takeover via CSS media query
...
Plans:
- [ ] 06-01-PLAN.md — Flat pricing card CSS overhaul: featured navy block, tier variants, price display, badges, CTA buttons
- [ ] 06-02-PLAN.md — Step header accent, geometric circle decorators, flat gray-900 CTA section

### Phase 7: Flat Design Cleanup
**Goal**: Close the two flat design gaps identified by milestone audit — flatten the Step 2 `.info-section` container and remove the `border-radius` from `.feature-item.highlight` in Step 3
**Depends on**: Phase 6
**Requirements**: WIZD-05, CAMP-01, CAMP-02, CAMP-03, QUOT-02
**Gap Closure**: Closes gaps from v1.1 milestone audit
**Success Criteria** (what must be TRUE):
  1. `.info-section` in Step 2 has no `linear-gradient` background — solid flat color only
  2. `.info-section` has `border-radius: 0` — sharp corners throughout
  3. `.feature-item.highlight` in Step 3 has `border-radius: 0`
  4. No gradient or non-zero border-radius remains on any wizard element
**Plans**: 1 plan

Plans:
- [ ] 07-01-PLAN.md — Flatten .info-section and .feature-item.highlight (CSS gap closure)

### Phase 8: Wizard Shell Gap Closure
**Goal**: Close all remaining v1.1 audit gaps — eliminate the live Aura popover shadow (WIZD-05), constrain the mobile dialog footer (WIZD-02), add the missing font preconnect (perf), and remove dead CSS with latent border-radius risk
**Depends on**: Phase 7
**Requirements**: WIZD-05, WIZD-02, WIZD-01
**Gap Closure**: Closes gaps from v1.1-MILESTONE-AUDIT.md
**Success Criteria** (what must be TRUE):
  1. `.p-popover` renders with no box-shadow in Step 2 — `:deep(.p-popover)` override present in WizardView.vue scoped styles
  2. `.p-dialog-footer` has `flex-shrink: 0` inside the mobile media block — footer does not compress content area on mobile
  3. `index.html` has preconnect hints for both `fonts.googleapis.com` and `fonts.gstatic.com`
  4. `.save-quote-section` and `.save-quote-form` dead CSS selectors (and their dark mode duplicates) are removed from WizardView.vue
**Plans**: 1 plan

Plans:
- [ ] 08-01-PLAN.md — Popover shadow fix, mobile footer fix, font preconnect, dead CSS removal
...
| 3. Foundation | 2/2 | Complete   | 2026-03-03 | - |
| 4. Venue Selection | 2/2 | Complete   | 2026-03-03 | - |
| 5. Campaign Details + Mobile | 2/2 | Complete   | 2026-03-03 | - |
| 6. Quote & Pricing | 2/2 | Complete   | 2026-03-05 | - |
| 7. Flat Design Cleanup | 1/1 | Complete   | 2026-03-05 | - |
| 8. Wizard Shell Gap Closure | v1.1 | Complete    | 2026-03-05 | - |
